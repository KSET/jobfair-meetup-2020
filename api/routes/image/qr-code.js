...
router.getRaw("/for-uid/:uid.svg", async (req, res) => {
  ...
  const resume = await ResumeService.info(uid);
  ...
  try {
    ...
    const [
      ...
      x,
      y,
    ] = String(qrCode).match(/viewBox="(?<coords>.*?)"/i).groups?.coords.split(" ").map(Number);
    ...
    res.send(
      qrCode
        .replace(
          "</svg>",
          `
              <rect x="${ pos.x }" y="${ pos.y }" width="9" height="9" fill="#00003f"/>
